...
public class Jeu {
    ...
    public void isSunk(Boat bo, Player J1) {
        ...
        if (bo.getLenght() == indexHit) {
            ...
            // Mettre à jour l'ID du bateau à "Z " s'il est coulé
            for (int j = 0; j < bo.getLenght(); j++) {
                if (sens == 0) {
        ...
        for (int j = 0; j < bo.getLenght(); j++) {
            if (sens == 0) {
                if (J.getBoard().grilled[x][y + j].equals("Z ")) {
                    ...
                    return false;  // Le bateau n'est pas entièrement coulé
                }
            } else {
                if (J.getBoard().grilled[x + j][y].equals("Z ")) {
                    continue;  // Continuez à vérifier les autres parties du bateau
                } else {
                    return false;  // Le bateau n'est pas entièrement coulé
                }
            }
        }
        return true;  // Toutes les parties du bateau sont coulées
    }
